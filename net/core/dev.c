...
static inline void ____napi_schedule(struct softnet_data *sd,
				     struct napi_struct *napi)
{
	list_add_tail(&napi->poll_list, &sd->poll_list);//yjh add napi poll list to softnet_data poll
	__raise_softirq_irqoff(NET_RX_SOFTIRQ);//yjh set pending bit callback net_rx_action soft intr action in irq_exit
}
...
void __napi_schedule(struct napi_struct *n)
{
	...
	local_irq_save(flags); //disable local cpu intr
		    ...
		    int (*poll)(struct napi_struct *, int), int weight)
{
	...
	napi->gro_list = NULL;
	napi->skb = NULL;
	napi->poll = poll; //yjh callback
	if (weight > NAPI_POLL_WEIGHT)
		pr_err_once("netif_napi_add() called with weight %d on device %s\n",
			    weight, dev->name);
	napi->weight = weight; //yjh the ability to poll.
	list_add(&napi->dev_list, &dev->napi_list); //yjh add napi to dev napi->list, a dev has not only one napi, every ring has a napi
	napi->dev = dev;
...
static int napi_poll(struct napi_struct *n, struct list_head *repoll)
{
	...
	list_del_init(&n->poll_list); //yjh del n from the &list in net_rx_action
	...
	if (test_bit(NAPI_STATE_SCHED, &n->state)) {
		work = n->poll(n, weight);//yjh for igb callback igb_poll 
	...
	list_add_tail(&n->poll_list, repoll);//yjh n has been polled, add it's poll list to repoll
...
static __latent_entropy void net_rx_action(struct softirq_action *h)
{
	...
	int budget = netdev_budget; //yjh to stop poll.
	LIST_HEAD(list);
	LIST_HEAD(repoll);

	local_irq_disable();//yjh stop change sd->poll_list
	list_splice_init(&sd->poll_list, &list);
	local_irq_enable();

	for (;;) { //loop all dev poll list in softnet_data poll list
		...
		n = list_first_entry(&list, struct napi_struct, poll_list); //yjh napi
		...
		if (unlikely(budget <= 0 || 
			     time_after_eq(jiffies, time_limit))) {//yjh timeout
			sd->time_squeeze++;
			break;
		}
	}
...
int register_netdevice(struct net_device *dev)
{
	...
	if (((dev->hw_features | dev->features) & //insure the vlan ops
