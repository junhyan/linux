			      ...
			      int rxr_count, int rxr_idx)
{
	...
	q_vector = adapter->q_vector[v_idx]; //yjh alloc q_vector
	if (!q_vector) {
	...
	/* initialize NAPI yjh init napi->poll callback igb poll*/ 
	netif_napi_add(adapter->netdev, &q_vector->napi, //jyh add napi
		       igb_poll, 64);
	...
	if (txr_count) {
		...
		adapter->tx_ring[txr_idx] = ring;   //yjh map tx ring to the intr vector
	...
	if (rxr_count) {
		...
		/* assign ring to adapter */
		adapter->rx_ring[rxr_idx] = ring;   //yjh map rx ring to the intr vector
	}
...
static int igb_init_interrupt_scheme(struct igb_adapter *adapter, bool msix)
{
	...
	igb_set_interrupt_capability(adapter, msix);    //yjh set adapter->num_q_vectors = numvecs;

	err = igb_alloc_q_vectors(adapter); //yjh refer to the q_vectors' number alloc interrupt vector(igb_q_vector)
	if (err) {
...
static int igb_request_irq(struct igb_adapter *adapter)
{
	...
	err = request_irq(pdev->irq, igb_intr, IRQF_SHARED, //yjh request irq for igb, and handler igb_intr
			  netdev->name, adapter);
...
static int igb_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	err = -ENOMEM;
	netdev = alloc_etherdev_mq(sizeof(struct igb_adapter), //yjh init netdev and queues in the netdev
				   IGB_MAX_TX_QUEUES);
	if (!netdev)
	...
	adapter = netdev_priv(netdev);  //yjh adapter, igb info
	...
	err = igb_sw_init(adapter); //yjh igb init
	if (err)
		goto err_sw_init;
...
static int igb_sw_init(struct igb_adapter *adapter)
{
	...
	igb_probe_vfs(adapter); //yjh igb vf

	igb_init_queue_configuration(adapter);  //yjh multi queue configuration
	...
	if (igb_init_interrupt_scheme(adapter, true)) { //yjh init igb hw interrupt.
...
static int __igb_open(struct net_device *netdev, bool resuming)
{
	...
	err = igb_setup_all_tx_resources(adapter);//yjh setup tx 
	if (err)
		goto err_setup_tx;

	/* allocate receive descriptors */
	err = igb_setup_all_rx_resources(adapter);//yjh mark
	if (err)
		goto err_setup_rx;

	igb_power_up_link(adapter); //yjh dev power up
	...
	err = igb_request_irq(adapter); //yjh alloc hw intr to igb
	if (err)
		goto err_req_irq;
...
int igb_setup_rx_resources(struct igb_ring *rx_ring)
{
	...
	rx_ring->desc = dma_alloc_coherent(dev, rx_ring->size,  //yjh rx_ring->desc physical addr rx_ring->dma
...
static irqreturn_t igb_intr(int irq, void *data)
{
	...
	igb_write_itr(q_vector); //yjh disable dev intr by Interrupt Throttling
...
static int igb_poll(struct napi_struct *napi, int budget)
{
	...
	if (q_vector->rx.ring) {
		int cleaned = igb_clean_rx_irq(q_vector, budget); //yjh key function, poll from the 
	...
	igb_ring_irq_enable(q_vector); //yjh enable dev intr
...
static int igb_clean_rx_irq(struct igb_q_vector *q_vector, const int budget)
{
	...
	struct sk_buff *skb = rx_ring->skb; //yjh get skb from rx ring
	unsigned int total_bytes = 0, total_packets = 0;
	...
	while (likely(total_packets < budget)) {
		...
		igb_put_rx_buffer(rx_ring, rx_buffer);//yjh reserve the rx ring and rx_buff
		cleaned_count++;
		...
		if (igb_cleanup_headers(rx_ring, rx_desc, skb)) { //yjh my pad to 60 to the frame
			skb = NULL;
			continue;
		}
		...
		napi_gro_receive(&q_vector->napi, skb);//yjh deliver the skb
